...
# Функция для получения путей к известным папкам через GUID
...
# ctypes функция для получения путей через Windows API
...
def list_directory(directory: Union[str, int]) -> str:
    ...
    if not path or not os.path.exists(path):
        return f"Папка не найдена или доступ закрыт."
...
@mcp.tool
def execute_open_file(file_id: int) -> str:
    """
    Открывает файл или директорию по идентификатору из кэша.
    ...
    Note:
        - Использует кэш для получения пути к файлу.
...
def undo_last_action() -> str:
    """
    Отменяет последнее действие пользователя или ассистента (создание, переименование, копирование, перемещение).
