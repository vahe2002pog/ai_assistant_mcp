...
class CustomMassageManager(MessageManager):
    def __init__(
        ...
        # Кастомно: переносим информацию о задаче в state_message
    ...
    def cut_messages(self):
        """Получить текущий список сообщений, при необходимости урезанный до max_tokens"""
        ...
        while diff > 0 and len(self.history.messages) > min_message_len:
            self.history.remove_message(min_message_len)  # всегда удаляем самое старое сообщение
            diff = self.history.total_tokens - self.max_input_tokens
    ...
    def add_state_message(
        ...
        """Добавить состояние браузера как human-сообщение"""
        # добавляем state message и результат в следующее сообщение (оно не останется в памяти)
    ...
    def _count_text_tokens(self, text: str) -> int:
        if isinstance(self.llm, (ChatOpenAI, ChatAnthropic, DeepSeekR1ChatOpenAI)):
            try:
				...
				)  # Грубая оценка, если токенайзер недоступен
        else:
            tokens = (
				len(text) // self.estimated_characters_per_token
			)  # Грубая оценка, если токенайзер недоступен
    ...
    def _remove_state_message_by_index(self, remove_ind=-1) -> None:
        """Удалить последнее state-сообщение из истории"""
