...
class CustomAgent(Agent):
    def __init__(
        ...
        if self.model_name in ["deepseek-reasoner"] or "deepseek-r1" in self.model_name:
            # deepseek-reasoner не поддерживает function calling
            self.use_deepseek_r1 = True
            # deepseek-reasoner поддерживает только 64000 токенов контекста
        ...
        # извлечённый контент страниц
        self.extracted_content = ""
        # дополнительная информация для агента
        self.add_infos = add_infos
        # agent_state для остановки агента
    ...
    def _setup_action_models(self) -> None:
        """Настроить динамические модели действий из реестра контроллера"""
        # Получаем динамическую модель действий из реестра контроллера
        self.ActionModel = self.controller.registry.create_action_model()
        # Создаём модель вывода с динамическими действиями
        self.AgentOutput = CustomAgentOutput.type_with_custom_actions(self.ActionModel)

    def _log_response(self, response: CustomAgentOutput) -> None:
        """Логировать ответ модели"""
    ...
    def update_step_info(
        ...
        """
        Обновить информацию о текущем шаге
        """
    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
        """Получить следующее действие от LLM на основе текущего состояния"""
        messages_to_process = (
        ...
        # Ограничиваем количество действий максимально допустимым за шаг
    ...
    async def step(self, step_info: Optional[CustomAgentStepInfo] = None) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"🧠 Вся память: \n{step_info.memory}")
                self._save_conversation(input_messages, model_output)
                if self.model_name != "deepseek-reasoner":
                    # удаляем предыдущее сообщение
                    self.message_manager._remove_state_message_by_index(-1)
            except Exception as e:
                # вызов модели провалился — удаляем последнее state-сообщение из истории
                self.message_manager._remove_state_message_by_index(-1)
                raise e
            ...
            if len(result) != len(actions):
                # Что-то изменилось — LLM должен знать об этом
            ...
            if len(actions) == 0:
                # TODO: обработать случай без действий
                result = [
                    ActionResult(
                        is_done=True,
            ...
            for ret_ in result:
                if "Extracted page" in ret_.extracted_content:
                    # сохраняем каждую извлечённую страницу
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        """Выполнить задачу с максимальным числом шагов"""
        try:
            self._log_agent_run()

            # Выполняем начальные действия, если они заданы
            ...
            for step in range(max_steps):
                # 1) Проверяем, запрошена ли остановка
                ...
                # 2) Сохраняем последнее валидное состояние перед шагом
                ...
                logger.info("❌ Не удалось выполнить задачу за максимальное число шагов")
                if not self.extracted_content:
    ...
    def _create_stop_history_item(self):
        """Создать запись в истории при остановке агента."""
        try:
            # Пытаемся получить последнее валидное состояние из agent_state
            state = None
            if self.agent_state:
                last_state = self.agent_state.get_last_valid_state()
                if last_state:
                    # Преобразуем в BrowserStateHistory
            ...
            # Создаём финальную запись в истории агента со статусом done
            ...
            logger.error(f"Ошибка создания записи об остановке: {e}")
            # Создаём пустое состояние как запасной вариант
            state = self._create_empty_state()
            stop_history = AgentHistory(
                model_output=None,
                state=state,
    ...
    def create_history_gif(
        ...
        """Создать GIF из истории агента с наложенным текстом задачи и целей."""
        if not self.history.history:
            logger.warning("Нет истории для создания GIF")
            return

        images = []
        # если история пуста или первый скриншот None — GIF создать невозможно
        if not self.history.history or not self.history.history[0].state.screenshot:
            logger.warning("Нет истории или первого скриншота для создания GIF")
            return

        # Пробуем загрузить более красивые шрифты
        try:
            # Перебираем варианты шрифтов по приоритету
            ...
            for font_name in font_options:
                try:
                    if platform.system() == "Windows":
                        # На Windows нужно указывать абсолютный путь к шрифту
            ...
            if not font_loaded:
                raise OSError("Предпочтительные шрифты не найдены")
        ...
        # Загружаем логотип если запрошено
        logo = None
        if show_logo:
            try:
                logo = Image.open("./static/browser-use.png")
                # Уменьшаем логотип (высота ~40px)
                logo_height = 150
        ...
        # Создаём кадр с задачей если запрошено
        ...
        for i, item in enumerate(self.history.history, 1):
            ...
            # Конвертируем base64-скриншот в PIL Image
        ...
        if images:
            ...
            logger.warning("В истории не найдено изображений для создания GIF")
