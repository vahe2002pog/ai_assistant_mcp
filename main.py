"""
AI Ассистент — умный персональный помощник для Windows
...
# ── Системный промпт ──────────────────────────────────────────────────────────
_SYSTEM_PROMPT = """Ты — умный персональный ассистент для Windows.
...
def _ensure_apps_scanned() -> None:
    ...
    if count == 0:
        ...
        try:
            ...
            print(f"[Предупреждение] Ошибка сканирования: {e}")
            return
...
def _build_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="AI Ассистент — умный персональный помощник для Windows"
    )
