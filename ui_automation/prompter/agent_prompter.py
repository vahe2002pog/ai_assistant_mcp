...
from ui_automation.prompter.basic import BasicPrompter
...
class HostAgentPrompter(BasicPrompter):
    ...
    def __init__(
        ...
        """
        Инициализация `HostAgentPrompter`.
        :param is_visual: Запрос для визуальной модели?
        :param prompt_template: Путь к шаблону промпта.
        :param example_prompt_template: Путь к шаблону примеров.
        :param api_prompt_template: Путь к шаблону API.
        """
    ...
    def system_prompt_construction(self) -> str:
        """
        Формирует системный промпт для выбора действия/приложения.
    ...
    def user_prompt_construction(
        ...
        """
        Формирует пользовательский промпт для выбора действия.
        :param control_item: Элемент управления.
        :param prev_plan: Предыдущий план.
        :param prev_subtask: Предыдущая подзадача.
        :param user_request: Запрос пользователя.
        :param retrieved_docs: Найденные документы.
        :return: Строка промпта для выбора действия.
        """
    ...
    def user_content_construction(
        ...
        """
        Формирует содержимое пользователя для LLM.
        :param image_list: Список изображений.
        :param control_item: Элемент управления.
        :param prev_subtask: Предыдущая подзадача.
        :param prev_plan: Предыдущий план.
        :param user_request: Запрос пользователя.
        :param retrieved_docs: Найденные документы.
        :return: Список частей содержимого для LLM.
        """

        user_content = []

        if self.is_visual:
            screenshot_text = ["Текущие скриншоты:"]
    ...
    def examples_prompt_helper(
        ...
        """
        Формирует секцию примеров.
        :param header: Заголовок секции.
        :param separator: Разделитель между примерами.
        :return: Сформированная секция примеров.
        """
        template = """
        [Запрос пользователя]:
            {request}
    ...
    def api_prompt_helper(self, verbose: int = 1) -> str:
        """
        Формирует секцию описания API для промпта.
        :param verbose: Уровень подробности.
        :return: Сформированная секция API.
        """

        # Construct the prompt for APIs
        api_list = [
            "- Типы действий ограничены {actions}.".format(
...
class APIPromptLoader:
    ...
    def load_api_prompt(self) -> Dict[str, str]:
        """
        Load the prompt template for COM APIs + MCP sub-agent tools.
        :return: The merged prompt template.
        """
        result: Dict[str, str] = {}

        # App-specific API (Excel, Word, Web, etc.)
        prompt_address = configs["APP_API_PROMPT_ADDRESS"].get(self.root_name, None)
        if prompt_address:
            result.update(AppAgentPrompter.load_prompt_template(prompt_address, None))

        # MCP sub-agent tools — always available to all AppAgents
        mcp_prompt_address = configs.get("MCP_API_PROMPT", None)
        if mcp_prompt_address:
            mcp_prompt = AppAgentPrompter.load_prompt_template(mcp_prompt_address, None)
            if mcp_prompt:
                result.update(mcp_prompt)
