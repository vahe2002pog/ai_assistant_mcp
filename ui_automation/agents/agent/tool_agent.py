...
ИНСТРУМЕНТЫ:
  Запуск:       open_app(name)
                Для запуска приложения ищи ТОЛЬКО по смысловому имени/алиасу, не по путям.
                Примеры: «включи музыку» -> open_app("музыка"), «открой браузер» -> open_app("браузер").
                Не вызывай search_files/view_cache, чтобы найти exe/lnk приложения.
                Если open_app/list_apps не нашли приложение или действие в приложении не удалось
                для сервисного запроса (музыка, фото, видео, почта, карты, облако и т.п.),
                честно заверши task_done с причиной. Controller сможет перепланировать
                задачу через веб-сайт/веб-сервис. Не повторяй поиск приложения по путям.
