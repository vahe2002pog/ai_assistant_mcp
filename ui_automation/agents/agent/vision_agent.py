...
"""
    ...
    def execute(self, task: str, window_title: str = "") -> str:
        ...
        if attached_paths and images:
            try:
                return self._answer_attached_images(task, images)
            except Exception as e:
                return f"Ошибка LLM: не удалось проанализировать приложенное изображение: {e}"

        if not images:
            try:
                images.append({"b64": capture_base64(window_title), "mime": "image/png"})
            except Exception as e:
                print(f"  [VisionAgent] Не удалось сделать скриншот: {e}", flush=True)

        image_ok = bool(images)
    ...
    def _answer_attached_images(self, task: str, images: List[Dict[str, str]]) -> str:
        content: List[Dict[str, Any]] = [{
            "type": "text",
            "text": (
                "Пользователь приложил изображение файлом. Изображение уже передано тебе "
                "в этом сообщении как image_url. Не открывай его инструментами и не делай "
                "скриншот экрана. Рассмотри само приложенное изображение и ответь на запрос. "
                "Не говори, что изображение недоступно инструментам, если оно передано ниже.\n\n"
        ...
        messages: List[Dict[str, Any]] = [
            {
                "role": "system",
                "content": (
                    "Ты vision-модель. Отвечай только по приложенному изображению. "
                    "Если объект не виден или текст не читается, скажи это явно."
                ),
            },
