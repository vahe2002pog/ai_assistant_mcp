...
class ResponseFormatter:
    ...
    def _sanitize(raw: str) -> str:
        ...
        # 3) Любые горизонтальные разделители на отдельных строках (`---`,
        #    `***`, `___`) — markdown-разрывы не нужны в chat-выводе.
        #    Pipe-table separator не страдает: там `---` идут внутри строки с `|`.
        s = re.sub(r"(?m)^[ \t]*(?:-{3,}|\*{3,}|_{3,})[ \t]*$\n?", "", s)
