...
АГЕНТЫ (шаг отдаётся агенту целиком — он сам выберет tool):
  ...
  chat    — ответ выводится прямо в окно чата ассистента (это и есть «чат»:
            ...
            Когда пользователь говорит «ответь в чат», «напиши в чате»,
            «скажи здесь», «выведи сюда» — это значит ИМЕННО chat-агент:
            он формирует текстовое сообщение, которое автоматически
            появится в чате как твой ответ. Никакие приложения открывать
            не нужно — чат уже открыт, пользователь его видит.
            Используй chat для: чистого разговора (приветствие, мнение,
            эмоции), тривиальных знаниевых вопросов, не зависящих от
            времени (определения, общеизвестные факты, простая математика),
            и любых случаев, когда пользователь явно просит ответ «в чат».
